...
from RagBot.src.config import config
# from RagBot.src.logs import get_logger
...
class Retriever(object):
    ...
    # async def _rerank_documents(self, query: str, documents: list[str], k: int) -> list[str]:
    #     """
    ...
    async def _rerank_documents(self, query, documents, k):
        ...
        docs_with_scores = [(documents[i], scores[i]) for i in range(len(documents)) if
                            scores[i] > config["retriever"]["retriever_threshold"]]
...
if __name__ == '__main__':
    my_r = Retriever()
    query = "چطوری انبار"
    rr = asyncio.run(my_r.retrieve_context(query))
    print(rr)