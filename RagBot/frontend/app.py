"""
hamzan-ui — a thin FastAPI server.
 
Two jobs only:
  1. Serve the single-page UI (static/index.html).
  2. Proxy your existing backend API so the browser never talks to it
     directly. That avoids CORS errors and keeps BASE_URL server-side.
 
Everything below is plain Python. You should not need to touch the
frontend at all; just run this file.
 
Behind a path prefix (e.g. Istio routes /eval-front-soroush to this pod
WITHOUT rewriting the path) set FASTAPI_ROOT_PATH and every route, static
file, and the docs page automatically serve under that prefix.
 
Run locally (no prefix):
    pip install -r requirements.txt
    uvicorn app:app --reload --port 9092
    -> http://localhost:9092
 
Run the way your k8s Service expects (port 9092, served under the prefix):
    FASTAPI_ROOT_PATH=/eval-front-soroush \
        uvicorn app:app --host 0.0.0.0 --port 9092
    -> https://sg-da-dev.ml.abramad.com/eval-front-soroush/
"""
...
# Same config reference your other service uses. The Swagger UI assets are
# located through config["fastapi_local_files"]["path"] so both services point
# at one shared directory instead of each keeping its own copy.
 
load_dotenv()
 
# Your real backend. Set BASE_URL_BACKEND in a .env file next to this script,
# e.g.  BASE_URL_BACKEND=http://localhost:8000
BASE_URL = os.getenv("BASE_URL_BACKEND", "http://localhost:8000")
 
# How long to wait on your backend before giving up (seconds).
TIMEOUT = float(os.getenv("BACKEND_TIMEOUT", "120"))
 
# Path prefix this app is served under. Istio routes /eval-front-soroush to this
# pod WITHOUT rewriting the path, so the app MUST serve under that prefix.
# We default to it — and treat an empty value as "use the default" — so a bare
# `uvicorn app:app` always works behind the route, even if FASTAPI_ROOT_PATH is
# not exported. Set FASTAPI_ROOT_PATH only if you ever mount this service under
# a different prefix.
ROOT_PATH = os.getenv("UI_FASTAPI_ROOT_PATH") or "/eval-front-soroush"
 
# Name of the shared default Qdrant collection. Must match the backend's
# config["database"]["collection_name"]. Shown in the create-assistant form.
DEFAULT_COLLECTION = os.getenv("DEFAULT_COLLECTION", "default_collection_qwen3")
 
print(f"[hamzan-ui] root_path={ROOT_PATH!r}  backend={BASE_URL!r}  default_collection={DEFAULT_COLLECTION!r}", flush=True)
 
# Drop access-log lines for noisy poll paths. The old Streamlit frontend (still
# open in some browser tabs) polls /_stcore/health and /_stcore/host-config on a
# timer; this app has nothing there, so every poll logged a 404. Filtering keeps
# the logs readable. Add more substrings via QUIET_PATHS="/_stcore/,/foo".
QUIET_PATHS = tuple(p for p in os.getenv("QUIET_PATHS", "/_stcore/").split(",") if p)
...
class _QuietAccessFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        try:
            ...
            return True
        return not any(q in line for q in QUIET_PATHS)
...
# docs_url/redoc_url are disabled here because we serve our OWN /docs below,
# wired to local Swagger assets so it works with no internet access.
app = FastAPI(
    title="Digital Assistant — UI gateway",
    root_path=ROOT_PATH,
    docs_url=None,
    redoc_url=None,
)
...
class StripPrefixMiddleware:
    """Serve correctly behind Istio (prefix forwarded, NOT rewritten).
 
    Istio sends the full path /eval-front-soroush/... to this pod. This strips
    that prefix on the way IN so the routes — which are defined at /… — always
    match, for every URL form:
        /eval-front-soroush        -> /
        /eval-front-soroush/       -> /
        /eval-front-soroush/api/x  -> /api/x
    Paths that don't carry the prefix (e.g. a direct /healthz probe) pass
    through untouched. `root_path` still handles OUTBOUND URLs (docs links, the
    BASE injected into the page), so the two are complementary, not conflicting.
    This makes routing independent of root_path stripping, which removes the
    trailing-slash / empty-root_path 404 edge cases entirely.
    """
 
    def __init__(self, app, prefix: str):
        self.app = app
        p = "/" + prefix.strip("/")
        self.prefix = "" if p == "/" else p
...
# ---------------------------------------------------------------------------
# Offline Swagger docs. The assets are served from the SAME shared directory
# your other service uses (config["fastapi_local_files"]["path"]), so there is
# one reference, not a duplicate. The docs page never reaches a CDN.
# Visit  {prefix}/docs.
# ---------------------------------------------------------------------------
app.mount(
    "/swagger-static",
    StaticFiles(directory=os.getenv("UI_FASTAPI_LOCAL_FILES_PATH")),
    name="swagger-static",
)
...
async def custom_swagger_ui():
    root = app.root_path  # "" locally, "/eval-front-soroush" in the cluster
...
# ---------------------------------------------------------------------------
# Small helpers so every proxy route looks the same and fails the same way.
# ---------------------------------------------------------------------------
def _ok(resp: requests.Response):
    """Pass a backend response straight back to the browser as JSON."""
    try:
        data = resp.json()
    except ValueError:
        data = {"raw": resp.text}
...
def _fail(detail: str, status: int = 502):
    return JSONResponse(status_code=status, content={"error": detail})
 
 
# ---------------------------------------------------------------------------
# Proxy routes. Each one maps one frontend call to one of your endpoints.
# ---------------------------------------------------------------------------
@app.post("/api/session/create")
async def create_session(request: Request):
    body = await request.json()
    payload = {"tenant_name": "admin", "user_code": "admin"}
    if body.get("database_id"):
        payload["database_id"] = body["database_id"]
    try:
        r = requests.post(f"{BASE_URL}/v1/session/create", json=payload, timeout=TIMEOUT)
        return _ok(r)
    except requests.RequestException as e:
        return _fail(f"Could not reach the backend: {e}")
...
async def chat(request: Request):
    ...
    # Mirrors the backend ChatRequest schema exactly.
    chat_data = {
        "query": body.get("query"),
        "session_id": session_id,
        "does_evaluate": body.get("does_evaluate", False),
        "response_type": body.get("response_type", "concise"),
        "use_cache": body.get("use_cache", True),
        "on_click": body.get("on_click", False),
        "error_payload": body.get("error_payload", ""),
        "is_sync": body.get("is_sync", True),
        "model_name": body.get("model_name", ""),
        "sql_mode": body.get("sql_mode", False),
        "use_video_links": body.get("use_video_links", True),
        "ticket_mode": body.get("ticket_mode", True),
    }
    # The current ChatRequest binds the database to the session, so database_id
    # is only forwarded when present (harmless if the backend ignores it).
    if body.get("database_id"):
        chat_data["database_id"] = body["database_id"]
    try:
        r = requests.post(
            f"{BASE_URL}/v1/chat",
            json=chat_data,
            headers={"Session-ID": session_id or ""},
            timeout=TIMEOUT,
        )
        return _ok(r)
    except requests.RequestException as e:
...
async def history(session_id: str = Query(...)):
    ...
    try:
        r = requests.get(f"{BASE_URL}/v1/chat", params=params, timeout=TIMEOUT)
        return _ok(r)
    except requests.RequestException as e:
        return _fail(f"Could not reach the backend: {e}")
...
async def sessions():
    try:
        r = requests.get(f"{BASE_URL}/v1/sessions", timeout=TIMEOUT)
        return _ok(r)
    except requests.RequestException as e:
        return _fail(f"Could not reach the backend: {e}")
...
async def databases():
    try:
        r = requests.get(f"{BASE_URL}/v1/databases", timeout=TIMEOUT)
        return _ok(r)
    except requests.RequestException as e:
        return _fail(f"Could not reach the backend: {e}")
...
async def faq(
    ...
    num_retrieve_context: int = Query(5, ge=1, le=50),
):
    try:
        r = requests.get(
            f"{BASE_URL}/v1/faq",
...
async def create_database(
    ...
    recreate: bool = Query(True),
    batch_size: int = Query(100),
    default_collection: bool = Query(False),
    write_mode: str = Query("recreate"),
    ...
    try:
        # Re-stream the uploaded files to your backend.
        forward = [
            ("files", (f.filename, await f.read(), f.content_type or "application/octet-stream"))
            for f in files
        ]
        r = requests.post(
            f"{BASE_URL}/v1/chat/create/database",
            params=params,
            files=forward,
            timeout=max(TIMEOUT, 600),  # ingestion can be slow
        )
        return _ok(r)
    except requests.RequestException as e:
        return _fail(f"Could not reach the backend: {e}")
...
# ---------------------------------------------------------------------------
# Serve the UI. The page is served under the prefix automatically; we inject
# that prefix so the page's own fetch() calls hit /{prefix}/api/... and not
# /api/... (which would bypass the Istio route).  Keep this route LAST.
# ---------------------------------------------------------------------------
@app.get("/healthz", include_in_schema=False)
async def healthz():
    return {"status": "ok", "backend": BASE_URL, "root_path": app.root_path}
...
async def index():
    with open(os.path.join(STATIC_DIR, "index.html"), encoding="utf-8") as f:
        html = f.read()
    # Tell the frontend which prefix to put in front of every /api call.
    html = html.replace("__ROOT_PATH__", app.root_path or "")
    html = html.replace("__DEFAULT_COLLECTION__", DEFAULT_COLLECTION)
    # Never cache the page itself, so a stale copy can't linger in the browser.
    return HTMLResponse(html, headers={"Cache-Control": "no-store, max-age=0"})
 
 
# Serve any other static assets (kept after the routes above).
