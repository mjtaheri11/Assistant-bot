...
from .retriever import TritonEmbeddings
...
LIST_OF_VALID_MODEL_NAMES = ["logistic_regression", "svm", "mlp", "rf", "knn"]

YOUR_SITE_URL = "<YOUR_SITE_URL>"
YOUR_SITE_NAME = "<YOUR_SITE_NAME>"
MODEL_NAME = "qwen/qwen3-embedding-4b"
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY", default="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")
...
class SVMModel(ClassifierModel):
    ...
    def __load_model__(self, address=None):
        if address is None:
            self.model = SVC(kernel='rbf', C=2.0, probability=True)
        else:
            self.model = joblib.load(address)
...
class SemanticRouterPipeline:
    ...
    def __load_embedding_model(self, address, model=None):
        if (address is not None and os.path.exists(address)) or model is not None:
            ...
            elif model == "triton-e5":
                model_name = "e5"
                # triton_url = "https://triton-server.ml.abramad.com"
                # triton_url = "https://trt.ml.abramad.com"
                triton_url = "http://triton-server.admin.svc.cluster.local"
                tokenizer_path = r"/home/jovyan/Assistant-bot/RagBot/saved_models/models--intfloat--multilingual-e5-large/snapshots/0dc5580a448e4284468b8909bae50fa925907bc5"
                self.embedder = TritonEmbeddings(model_name=model_name,
                                                 triton_url=triton_url,
                                                 tokenizer_path=tokenizer_path)
            elif model == "triton-qwen3-embedding-4b":
                model_name = "qwen3"
                # triton_url = "https://triton-server.ml.abramad.com"
                # triton_url = "https://trt.ml.abramad.com"
                triton_url = "http://qwen3-embedding-soroush.cs-sg-da-dev.svc.cluster.local:82"
                tokenizer_path = r"/home/jovyan/.cache/huggingface/hub/models--Qwen--Qwen3-Embedding-4B/snapshots/5cf2132abc99cad020ac570b19d031efec650f2b"
                self.embedder = TritonEmbeddings(model_name=model_name,
                                                 triton_url=triton_url,
                                                 tokenizer_path=tokenizer_path)
            else:
    ...
    def __load_classifier_model(self, classifier_address, **kwargs):
        ...
        elif kwargs.get("model_name") == "knn":
            self.classifier = KNNClassifierModel(classifier_address)
        else:
    ...
    def __train_model__(self, **kwargs):
        ...
        if model_name == "logistic_regression":
            ...
            self.classifier = KNNClassifierModel(address=None)
    ...
    def __load_train_data_embeddings__(self, **kwargs):
        ...
        if train_embeddings_address is not None and os.path.isfile(train_embeddings_address):
            ...
            embeddings = self.embedder.embed_documents(self.X)
            print("length of embeddings is: %s" % len(embeddings))
...
if __name__ == '__main__':
    dataset_address = "/home/jovyan/Assistant-bot/RagBot/resources_for_router/output_with_augmentation_big_ones.xlsx"
    embedding_address = r"/home/jovyan/.cache/huggingface/hub/models--Qwen--Qwen3-Embedding-4B/snapshots/5cf2132abc99cad020ac570b19d031efec650f2b"
    semantic_router_object = SemanticRouterPipeline(inference_only=False,
                                                    dataset_address=dataset_address,
                                                    train_embeddings_address=(Path("/home/jovyan/Assistant-bot/RagBot/resources_for_router/vectors_qwen3_embedding_4b") / "embeddings.npy").as_posix(),
                                                    # train_embeddings_address=None,
                                                    ...
                                                    embedding_model="triton-qwen3-embedding-4b",
                                                    save_train_model_address="/home/jovyan/Assistant-bot/RagBot/resources_for_router/models_multilingual_e5_large")
    print("Done!")
    exit()
    total_time = 0
    for i in range(10):
        t0 = time()
        classifier_address = r"E:\semantic_router\resources\models\classifiers_multilingual_e5_large_big\svm.joblib"
        embedding_address = r"E:\semantic_router\resources\vectors_qwen3_embedding_4b_augmented\embeddings.npy"
        ...
        #                                                 model_name="mlp",
        #                                                 dataset_address=dataset_address)
        semantic_router_object = SemanticRouterPipeline(inference_only=True,
                                                        embedding_address=embedding_address,
                                                        # embedding_model="qwen3-embedding-4b",
                                                        embedding_model="multilingual-e5-large",
                                                        classifier_address=classifier_address,
        ...
        new_sentence = "چطوری سند بزنم"
        output = semantic_router_object.predict_sentences([new_sentence])
        t2 = time()
