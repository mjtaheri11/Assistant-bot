...
def chunk_document(doc_paths: List[str], target_chunk_size: int, max_chunk_size: int) -> List[Document]:
    """
...
def finalize_chunk(chunk, target_chunk_size, max_chunk_size, source_file):
    """
    Finalize a chunk, splitting it if necessary based on sentences and paragraphs, with specific-sentence overlap.
    ...
    # Create a list of headers that are not empty
    header_list = []
    # import pdb
    # pdb.set_trace()

    if chunk['h1'].strip():
        header_list.append(chunk['h1'].strip())

    if chunk['h2'].strip():
        header_list.append(chunk['h2'].strip())

    if chunk['h3'].strip():
        header_list.append(chunk['h3'].strip())

    if chunk['h4'].strip():
        header_list.append(chunk['h4'].strip())

    # Join the headers with the separator
    headers = separator.join(header_list)

    # Now headers will have the desired format
    headers += "\n"
    # all_sentences = []
    ...
    for i, sentence in enumerate(all_sentences):
        ...
        if current_size >= target_chunk_size and i < len(all_sentences) - 1:
            ...
            # Start new chunk with x-sentence overlap
            overlap_sentences = current_chunk_sentences[-config["retriever"]["sentence_overlap"]:] if len(current_chunk_sentences) >= config["retriever"]["sentence_overlap"] else current_chunk_sentences[-1:]
