...
from styles import  HTML_RTL_INPUT_TITLE, HTML_RTL_INPUT_BODY, HTML_STYLE_FOR_RTL_INPUT_ELEMENT
...
def main():
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            ...
            if len(st.session_state.contexts) > config['retriever']['max_context_length']:
                reset_conversation()
            ...
            for response in llm.stream(RAG_SYSTEM_PROMPT.format(context=contexts, history=history, question=user_query)):
                full_response += response.content
                message_placeholder.markdown(HTML_RTL_INPUT_BODY.format(full_response + "▌"),unsafe_allow_html=True, )
            message_placeholder.markdown(HTML_RTL_INPUT_BODY.format(full_response),unsafe_allow_html=True)

        st.session_state.messages.append({"role": "assistant", "content": full_response})
        logger.info("call llm", extra={"session": ctx.session_id, 'question': user_query, 'answer': full_response, 'context': context, 'history': history})
    ...
    if st.session_state.sidebar_visible:
        with st.sidebar:
            st.markdown(HTML_RTL_INPUT_TITLE.format('مطالب مرتبط با سوال شما'), unsafe_allow_html=True)
